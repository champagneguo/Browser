...
public abstract class BaseUi implements UI {
    ...
    public BaseUi(Activity browser, UiController controller) {
        ...
        mTitleBar = new TitleBar(mActivity, mUiController, this,
                mContentView);
        mFakeDimLayer = mFrameLayout.findViewById(R.id.dim_layer);
        mFakeDimLayer.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (mBottomBar.isExpanded()){
                    mBottomBar.showMore(false);
                }
                if (mSideBar.isShowing()){
                    mSideBar.hide(true);
                }
            }
        });
        mSideBar = new SideBar(mActivity, mUiController, this, 
            (FrameLayout)mFrameLayout.findViewById(R.id.nt_content_container), false);
        mSideBar.setFakeDimLayerView(mFakeDimLayer);
        /// M: add bottom bar @{
        ...
        if (mNeedBottomBar) {
            mBottomBar = new BottomBar(mActivity, mUiController, this, mTabControl,
				//mContentView); //ntian modified
            (FrameLayout)mFrameLayout.findViewById(R.id.nt_content_container));
            mBottomBar.setFakeDimLayerView(mFakeDimLayer);
        }
        /// @}
        ...
        mTabControl.setOnTabCountChangedListener(new TabControl.OnTabCountChangedListener() {
            public void onTabCountChanged() {
                if (mBottomBar != null) mBottomBar.tabCountChanged(mTabControl.getTabCount());
                if (mTitleBar != null) mTitleBar.tabCountChanged(mTabControl.getTabCount());
            }
        });
    }
    ...
    public boolean onMenuKey() {
        if (!mBottomBar.isShowing()){
            ...
            mBottomBar.showMore(!mBottomBar.isExpanded());
        }

        if (mSideBar.getCanShow()){
            if (mSideBar.isShowing()){
                mSideBar.hide(true);
            }else{
                mSideBar.show(true);
    ...
    public void setActiveTab(final Tab tab) {
        ...
        // Keep 2 tabs' view in content view at most, remove others.
        for (int i = 0; i < mContentView.getChildCount(); i++){
            View container = mContentView.getChildAt(i);
            Tab tmp = (Tab)container.getTag();
            if (tmp != null && tmp != mActiveTab) {
                removeTabFromContentView(tmp);
            }
        }

        if ((tab != mActiveTab) && (mActiveTab != null)) {
            mLastActiveTab = mActiveTab;
    ...
    protected void showFakeDimEffect(boolean show) {
        if (mActivity instanceof NTStatusBarFakeDimActivity){
                ((NTStatusBarFakeDimActivity)mActivity).setStatusBarFakeDim(show);
        }
        mFakeDimLayer.setVisibility(show ? View.VISIBLE : View.GONE);
    }
