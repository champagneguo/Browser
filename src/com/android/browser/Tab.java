...
class Tab implements PictureListener {
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public void onProgressChanged(WebView view, int newProgress) {
            ...
            if (newProgress == 100) {
                mInPageLoad = false;
                View tmp = mContainer.findViewById(R.id.first_look);
                if (tmp != null && tmp.getVisibility() == View.VISIBLE){
        ...
        public void onGeolocationPermissionsShowPrompt(String origin,
            ...
            if (mInForeground) {
                mCurrOrigin = origin;
                mGeoPermissionCallback = callback;
                //getGeolocationPermissionsPrompt().show(origin, callback);
                Uri uri = Uri.parse(origin);
                String message = String.format(mContext.getResources().getString(R.string.geolocation_permissions_prompt_message),
                    "http".equals(uri.getScheme()) ?  origin.substring(7) : origin);
                AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
                LayoutInflater factory = LayoutInflater.from(mContext);
                final View vw = factory.inflate(R.layout.nt_geolocation_permissions_prompt, null);
                final CheckBox cbRemember = (CheckBox)vw.findViewById(R.id.remember);
                         ...
                         if (which == DialogInterface.BUTTON_NEGATIVE) {
                             ...
                             if (remember) {
                                 Toast toast = Toast.makeText(
                                     mContext, R.string.geolocation_permissions_prompt_toast_disallowed, Toast.LENGTH_LONG);
                                 toast.setGravity(Gravity.BOTTOM, 0, 0);
                                 toast.show();
                             }

                             mGeoPermissionCallback.invoke(mCurrOrigin, false, remember);
                         }
                     }
                });
                builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                     ...
                     public void onClick(DialogInterface dialog, int which) {
                         if (which == DialogInterface.BUTTON_POSITIVE) {
                             ...
                             if (remember) {
                                 Toast toast = Toast.makeText(
                                     mContext, R.string.geolocation_permissions_prompt_toast_allowed, Toast.LENGTH_LONG);
                                 toast.setGravity(Gravity.BOTTOM, 0, 0);
                                 toast.show();
                             }

                             mGeoPermissionCallback.invoke(mCurrOrigin, true, remember);
                         }
                     }
                });

                final AlertDialog dialog = builder.create();
                ((TextView)vw.findViewById(R.id.message)).setText(message);
                dialog.show();
            }
        }
