...
public class Controller
    ...
    public Controller(Activity browser) {
        ...
        /// M: add for save page: // @ {
        HandlerThread updateSavePageThread = new HandlerThread("save_page");
        updateSavePageThread.start();
        mSavePageHandler = new UpdateSavePageDBHandler(updateSavePageThread.getLooper());
...
//          case KeyEvent.KEYCODE_Z:    // unused
        ...
        public void handleMessage(Message m) {
            ...
            switch (m.what) {
                ...
                case FAIL_SAVE_PAGE:
                    ...
                    builder.setContentTitle(mActivity.getText(R.string.saved_page_fail) + title).setContentIntent(null).setAutoCancel(true)
                        .setOngoing(false).setSmallIcon(R.drawable.ic_save_page_notification_fail);
    ...
    /// @ }
    private void animateOpenTab(Tab tab, String url){
        if (tab == null) return;
        ...
        if (parent == null){
            // The tab's view is not added to content view yet, we can't perform animation. So load url and return.
            if (url != null) {
                loadUrl(tab, url);
            }
            return;
        ...
        setBlockEvents(true);
        final View firstLookView = container.findViewById(R.id.first_look);
        final Tab targetTab = tab;
        final String URL = url;
        firstLookView.setVisibility(View.VISIBLE);
        final float startScale = 0.8f;
        container.setScaleX(startScale);
        container.setScaleY(startScale);

        ObjectAnimator transY = ObjectAnimator.ofFloat(container, "translationY", parent.getHeight(), 0);
        transY.setDuration(300);
        ObjectAnimator animScaleX = ObjectAnimator.ofFloat(container, "scaleX", startScale, 1.0f);
        ObjectAnimator animScaleY = ObjectAnimator.ofFloat(container, "scaleY", startScale, 1.0f);
        AnimatorSet scaleSet = new AnimatorSet();
        scaleSet.playTogether(animScaleX, animScaleY);
        scaleSet.setDuration(300);
        AnimatorSet combo = new AnimatorSet();
        combo.playSequentially(transY, scaleSet);
        combo.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator anim) {
                //firstLookView.setVisibility(View.GONE);
                setBlockEvents(false);
                if (URL != null) loadUrl(targetTab, URL);
            }
        });
        combo.start();
    }
