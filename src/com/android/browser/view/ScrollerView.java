...
public class ScrollerView extends FrameLayout {
    ...
    private static final boolean SCROLL_LIKE_VIEW_PAGER = true;
    protected PageChangedListener mPageChangedListener;
    private int mCurrPage = -1;

    public interface PageChangedListener { // used for SCROLL_LIKE_VIEW_PAGER is true
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mIsOrthoDragged) {
                    ...
                    if (getChildCount() > 0) {
                        if (SCROLL_LIKE_VIEW_PAGER){
                            mIsBeingDragged = false;
                            determineTargetScrollX((int)velocityTracker.getXVelocity(mActivePointerId));
                        }else{
    ...
    private void determineTargetScrollX(int velocity) {
        int pageWidth = getMeasuredWidth() - getPaddingLeft() - getPaddingRight();
        //if (pageWidth == 0) return;
        int targetPage = (pageWidth/2 + mScrollX)/pageWidth;
        ...
        int newScrollX = pageWidth * targetPage;
        smoothScrollTo(newScrollX, 0);
    }

    public void setPageChangedListener(PageChangedListener listener){ mPageChangedListener = listener; }
    ...
    private void checkIfPageChanged(){
        if (!mIsBeingDragged && mPageChangedListener != null){
            // notify page changed because of scroll
            int pageWidth = getMeasuredWidth() - getPaddingLeft() - getPaddingRight();
            int stoppedAtPage = (pageWidth / 2 + mScrollX) / pageWidth;
            if(mCurrPage != stoppedAtPage){
                mPageChangedListener.onPageChanged(stoppedAtPage);
            }
            mCurrPage = stoppedAtPage;
        }
    }
}
