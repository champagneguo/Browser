...
public class SiteNavigationProvider extends ContentProvider {
	...
	private static final UriMatcher S_URI_MATCHER = new UriMatcher(
			UriMatcher.NO_MATCH);
	static {
		S_URI_MATCHER.addURI(SiteNavigation.AUTHORITY, "websites",
				WEB_SITES_ALL);
		S_URI_MATCHER.addURI(SiteNavigation.AUTHORITY, "websites/#",
				WEB_SITES_ID);
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case WEB_SITES_ALL:
			break;
		case WEB_SITES_ID:
			qb.appendWhere(SiteNavigation.ID + "="
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case WEB_SITES_ALL:
			count = db
					.update(TABLE_WEB_SITES, values, selection, selectionArgs);
			break;
		case WEB_SITES_ID:
			String newIdSelection = SiteNavigation.ID
					+ "="
					+ uri.getLastPathSegment()
					+ (!TextUtils.isEmpty(selection) ? " AND (" + selection
							+ ')' : "");
			try {
				count = db.update(TABLE_WEB_SITES, values, newIdSelection,
						selectionArgs);
	...
	public ParcelFileDescriptor openFile(Uri uri, String mode) {
		try {
			...
			new RequestHandlerSiteNavigation(getContext(), uri,
					afd.createOutputStream()).start();
			return pipes[0];
		} catch (IOException e) {
	...
	private class SiteNavigationDatabaseHelper extends SQLiteOpenHelper {
		...
		public SiteNavigationDatabaseHelper(Context context) {
			super(context, DATABASE_NAME, null, 1); // "1" is the db version
													// here
		...
		private void createTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE websites (" + SiteNavigation.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
					+ SiteNavigation.URL + " TEXT," + SiteNavigation.TITLE
					+ " TEXT," + SiteNavigation.DATE_CREATED + " LONG,"
		...
		private void initTable(SQLiteDatabase db) {
			...
			mBrowserSiteNavigationExt = Extensions
					.getSiteNavigationPlugin(mContext);
			if (null == (websites = mBrowserSiteNavigationExt
					.getPredefinedWebsites())) {
				websites = mContext.getResources().getTextArray(
						R.array.predefined_websites_default_optr);
			}
			...
			if (!mContext.getResources().getBoolean(R.bool.isTablet)) {
				siteNavigationWebsiteNum = mBrowserSiteNavigationExt
						.getSiteNavigationCount();
			...
			try {
				for (int i = 0; i < websiteSize; i = i + 3) {
					...
					CharSequence websiteDestination = BrowserProvider2
							.replaceSystemPropertyInString(mContext,
									websites[i + 1]);
					values.put(SiteNavigation.URL,
							websiteDestination.toString());
				...
				for (int i = websiteSize / 3; i < siteNavigationWebsiteNum; i++) {
					...
					bm.compress(Bitmap.CompressFormat.PNG, 100, os);
					values = new ContentValues();
					// Url must be unique, because we use this column to query
					// and update
					// The number after about:blank is just the id in the
					// database
